...
func create(w http.ResponseWriter, r *http.Request){
	...
	c := session.DB("dojogo").C("people")
	err = c.Insert(&Person{usuario.Name, usuario.Phone, usuario.Id, usuario.Email, usuario.Direccion})
	if err != nil {
					log.Fatal(err)
	}
...
func getByPhone(w http.ResponseWriter, r *http.Request){
	vars := mux.Vars(r)
	name := vars["phone"]
	session, err := mgo.Dial("mongodb:27017/dojogo")
	if err != nil {
	...
	var usuarios []Person
	c := session.DB("dojogo").C("people")
	err = c.Find(bson.M{"Phone": phone}).All(&usuarios)
	respuesta, err :=  json.Marshal(usuarios)
	w.Header().Set("Content-Type", "application/json")
	w.WriteHeader(200)
	w.Write(respuesta)
...
func main(){
	...
	r.HandleFunc("/user/{phone}", getByPhone).Methods("GET")
